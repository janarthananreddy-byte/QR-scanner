...
// ✅ FIX 1: Use /tmp on Vercel (writable), local path otherwise
...
// ✅ FIX 2: Serve public/ AND root (for index.html at repo root)
...
// ✅ FIX 3: Accept CC + any number of digits (not just 8)
if (!/^CC\d+$/.test(code)) {
return res.status(400).json({ error: `Invalid code "${code}". Expected format: CC followed by digits (e.g. CC00000001)` });
}
